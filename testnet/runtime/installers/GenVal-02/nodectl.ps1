...
function Reset-Chain {
  ...
  foreach ($target in @(
    $ChainDir,
    (Join-Path $DataDir "testnet15\$($NodeEnv['NODE_SLOT_ID'])\chain"),
    (Join-Path $DataDir "testnet15\$($NodeEnv['NODE_SLOT_ID'])\logs"),
  ...
  New-Item -ItemType Directory -Force -Path (Join-Path $DataDir "testnet15\$($NodeEnv['NODE_SLOT_ID'])\chain") | Out-Null
  New-Item -ItemType Directory -Force -Path (Join-Path $DataDir "testnet15\$($NodeEnv['NODE_SLOT_ID'])\logs") | Out-Null
