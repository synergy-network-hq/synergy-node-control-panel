...
reset_chain() {
  stop_node || true
  rm -rf "$CHAIN_DIR" "$DATA_DIR/testnet15/$NODE_SLOT_ID/chain" "$DATA_DIR/testnet15/$NODE_SLOT_ID/logs"
  rm -f "$DATA_DIR/chain.json" "$DATA_DIR/token_state.json" "$DATA_DIR/validator_registry.json" "$DATA_DIR/committed_qcs.json" "$DATA_DIR/canonical_locks.json" "$DATA_DIR/dag_state.json" "$DATA_DIR/synergy-testnet.pid" "$DATA_DIR/.reset_flag" "$PID_FILE"
  mkdir -p "$CHAIN_DIR" "$LOG_DIR" "$DATA_DIR/testnet15/$NODE_SLOT_ID/chain" "$DATA_DIR/testnet15/$NODE_SLOT_ID/logs"
