...
pub const TESTBETA_AGENT_PORT: u16 = 47_990;
const DEFAULT_REMOTE_ROOT_UNIX: &str = "/opt/synergy";
const DEFAULT_REMOTE_ROOT_WINDOWS: &str = "C:\\Synergy\\Testnet-Beta";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TestnetBetaAgentHealth {
    pub status: String,
    pub version: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TestnetBetaAgentControlRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TestnetBetaAgentControlResponse {
    pub node_slot_id: String,
    pub action: String,
    pub success: bool,
    pub exit_code: i32,
...
struct AgentState {
    ...
    jobs: Arc<Mutex<HashMap<String, TestnetBetaAgentControlResponse>>>,
}
...
pub async fn serve_with_host(
    ...
    eprintln!(
        "[{}] testbeta-agent starting workspace={} port={} host={}",
        Utc::now().to_rfc3339(),
    ...
    for bind_addr in bind_addresses_for_host(host.as_deref(), port) {
        match tokio::net::TcpListener::bind(bind_addr).await {
            ...
            Err(error) if bind_addr.ip().is_loopback() || bind_addr.ip().is_unspecified() => {
                return Err(format!(
                    "Failed to bind testbeta agent on {bind_addr}: {error}"
                ));
            }
            Err(error) => {
                eprintln!("testbeta agent optional bind skipped on {bind_addr}: {error}");
            }
        }
    }

    if listeners.is_empty() {
        return Err("Failed to bind testbeta agent on loopback or VPN interface".to_string());
    }
    ...
    for (bind_addr, listener) in listeners {
        ...
        servers.spawn(async move {
            axum::serve(listener, service)
                .await
                .map_err(|error| format!("Testnet-Beta agent server error on {bind_addr}: {error}"))
        });
    }
    ...
    while let Some(result) = servers.join_next().await {
        match result {
            ...
            Err(error) => return Err(format!("Testnet-Beta agent server task panicked: {error}")),
        }
    }
...
async fn control_handler(
    ConnectInfo(remote_addr): ConnectInfo<SocketAddr>,
    State(state): State<AgentState>,
    Json(input): Json<TestnetBetaAgentControlRequest>,
) -> impl IntoResponse {
    if !is_allowed_remote(remote_addr.ip()) {
        return (
            StatusCode::FORBIDDEN,
    ...
    eprintln!(
        "[{}] testbeta-agent control node={} action={} remote={}",
        Utc::now().to_rfc3339(),
    ...
    if should_run_as_job(&normalized_action) {
        ...
        let queued = TestnetBetaAgentControlResponse {
        ...
        tokio::spawn(async move {
            ...
            let final_response = match job_result {
                ...
                Ok(Err(error)) => TestnetBetaAgentControlResponse {
                ...
                Err(join_error) => TestnetBetaAgentControlResponse {
                    node_slot_id: queued_for_job.node_slot_id.clone(),
                    action: queued_for_job.action.clone(),
                    success: false,
                    exit_code: 1,
...
fn execute_control(
    workspace_root: &Path,
    input: TestnetBetaAgentControlRequest,
    ...
    let result = match normalized_action.as_str() {
        ...
        other => Err(format!("Unsupported testbeta agent action: {other}")),
    }?;
    ...
    Ok(TestnetBetaAgentControlResponse {
        node_slot_id,
...
fn reset_chain(workspace_root: &Path, install: &NodeInstall) -> Result<CommandOutcome, String> {
    ...
    let node_data_dir = data_dir.join("testbeta15").join(&install.node_slot_id);
    let targets = [
        data_dir.join("chain"),
        ...
        data_dir.join("token_state.json"),
        data_dir.join("validator_registry.json"),
        data_dir.join("synergy-testbeta.pid"),
