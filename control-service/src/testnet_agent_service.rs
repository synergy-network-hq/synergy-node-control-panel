...
pub async fn serve_with_host(
    ...
    if listeners.is_empty() {
        return Err("Failed to bind testnet agent on loopback or VPN interface".to_string());
    }
...
fn bind_addresses_for_host(host: Option<&str>, port: u16) -> Vec<SocketAddr> {
    if let Some(host) = host {
        ...
        eprintln!("testnet agent: invalid --host value '{host}', falling back to 0.0.0.0");
    }
...
fn allow_public_agent_access() -> bool {
    std::env::var("SYNERGY_TESTNET_AGENT_ALLOW_PUBLIC")
        .ok()
        .map(|value| {
            matches!(
