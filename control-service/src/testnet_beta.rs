...
pub async fn testbeta_setup_node(
    ...
    if matches!(role.id.as_str(), "rpc_gateway" | "indexer") {
        ...
        let nginx_contents = if role.id == "rpc_gateway" {
            let rpc_subdomain = "testbeta-core-rpc.synergy-network.io";
            let ws_subdomain = "testbeta-core-ws.synergy-network.io";
...
fn provisioned_node_uses_private_validator_mesh(node: &TestnetBetaProvisionedNode) -> bool {
