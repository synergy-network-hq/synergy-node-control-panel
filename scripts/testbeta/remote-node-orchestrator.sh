...
if [[ -z "$SSH_USER" ]]; then
  SSH_USER="${SYNERGY_TESTBETA_SSH_USER:-ops}"
fi

SSH_PORT="$(resolve_var "$SSH_PORT_VAR")"
if [[ -z "$SSH_PORT" ]]; then
  SSH_PORT="${SYNERGY_TESTBETA_SSH_PORT:-22}"
fi
...
rm -rf data/chain data/testbeta15/'$NODE_SLOT_ID'/chain
rm -rf data/testbeta15/'$NODE_SLOT_ID'/logs
rm -f  data/chain.json data/token_state.json data/validator_registry.json
rm -f  data/synergy-testbeta.pid data/.reset_flag
rm -f  data/node.pid
...
./bin/synergy-testbeta-\$(uname -s | tr A-Z a-z)-\$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/') keygen --type vrf --output keys/ 2>/dev/null || {
...
CHAIN_DIR=data/testbeta15/$NODE_SLOT_ID/chain
if [[ ! -d \"\$CHAIN_DIR\" ]]; then
...
deploy_agent() {
  if [[ "$IS_LOCAL_TARGET" -eq 1 ]]; then
    echo "Node $NODE_SLOT_ID is on the local machine — the testbeta agent is managed by the Electron app and local control-service here."
  ...
  local binary_src="$ROOT_DIR/binaries/synergy-testbeta-agent-${platform_suffix}${binary_ext}"
  ...
  local agent_dir="/opt/synergy/testbeta-agent"
  local agent_bin="$agent_dir/synergy-testbeta-agent${binary_ext}"
...
if command -v systemctl >/dev/null 2>&1 && [[ -d /etc/systemd/system ]]; then
  ...
  SERVICE_FILE=/etc/systemd/system/synergy-testbeta-agent.service
  UNIT_CONTENT=\"[Unit]
...
SyslogIdentifier=synergy-testbeta-agent
...
WantedBy=multi-user.target\"
  ...
  if [[ \$(id -u) -eq 0 ]]; then
    ...
    systemctl enable synergy-testbeta-agent
    systemctl restart synergy-testbeta-agent
    sleep 2
    systemctl status synergy-testbeta-agent --no-pager || true
  else
    echo \"\$UNIT_CONTENT\" | sudo tee \"\$SERVICE_FILE\" >/dev/null
    sudo systemctl daemon-reload
    sudo systemctl enable synergy-testbeta-agent
    sudo systemctl restart synergy-testbeta-agent
    sleep 2
    sudo systemctl status synergy-testbeta-agent --no-pager || true
  fi
  echo 'Agent installed as systemd service and started.'

else
  # Non-systemd fallback: kill old agent, start fresh in background.
  pkill -f 'synergy-testbeta-agent' 2>/dev/null || true
  sleep 1
  LOG_FILE='/var/log/synergy-testbeta-agent.log'
  touch \"\$LOG_FILE\" 2>/dev/null || LOG_FILE=\"\$HOME/synergy-testbeta-agent.log\"
