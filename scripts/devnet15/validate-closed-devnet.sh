...
CONFIG_DIR="${1:-$ROOT_DIR/devnet/lean15/configs}"
...
for config in "$CONFIG_DIR"/node-*.toml; do
  ...
  if ! rg -q '^name = "synergy-devnet-closed"$' "$config"; then
    echo "[$name] network name is not synergy-devnet-closed" >&2
    failures=$((failures + 1))
  fi
...
if (( failures > 0 )); then
  echo "Closed-devnet validation failed (${failures} issue(s))." >&2
  exit 2
fi

echo "Closed-devnet validation passed (${checked} config files checked, validators=${validator_count})."
