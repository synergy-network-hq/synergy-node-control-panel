...
NODE_ADDRESSES_FILE="$ROOT_DIR/testnet/runtime/keys/node-addresses.csv"
...
TESTNET_CHAIN_ID="${TESTNET_CHAIN_ID:-1263}"
...
TESTNET_VALIDATOR_CLUSTER_SIZE="${TESTNET_VALIDATOR_CLUSTER_SIZE:-7}"
TESTNET_VALIDATOR_VOTE_THRESHOLD="${TESTNET_VALIDATOR_VOTE_THRESHOLD:-4}"
TESTNET_MAX_VALIDATORS="${TESTNET_MAX_VALIDATORS:-100}"
...
TESTNET_P2P_HEARTBEAT_INTERVAL_SECS="${TESTNET_P2P_HEARTBEAT_INTERVAL_SECS:-5}"
...
while IFS=, read -r node_slot_id node_alias role_group role node_type _ p2p_port rpc_port ws_port grpc_port discovery_port host management_host physical_machine_id auto_register enable_pruning vrf_enabled operator device operating_system public_ip local_ip || [[ -n "${node_slot_id:-}" ]]; do
  ...
  host="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "HOSTNAME" "$host")"
  public_ip="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "PUBLIC_IP" "$public_ip")"
  local_ip="$(testnet_inventory_env_value_allow_empty "$node_slot_id" "$node_type" "$validator_address" "$host" "LOCAL_IP" "$local_ip")"
  bind_ip="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "BIND_IP" "")"
  p2p_port="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "P2P_PORT" "$p2p_port")"
  rpc_port="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "RPC_PORT" "$rpc_port")"
  ws_port="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "WS_PORT" "$ws_port")"
  grpc_port="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "GRPC_PORT" "$grpc_port")"
  discovery_port="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "DISCOVERY_PORT" "$discovery_port")"

  management_host="$(testnet_first_nonempty \
    "$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "MANAGEMENT_HOST" "")" \
    "$local_ip" \
    "$management_host" \
    "$public_ip" \
    "$host" \
  ...
  public_p2p_port="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "P2P_PORT_EXTERNAL" "$(resolve_public_p2p_port "$validator_address" "$p2p_port")")"
  public_address="$(compute_public_address "$resolved_public_host" "$public_p2p_port")"
  public_discovery_port="$(testnet_inventory_env_value "$node_slot_id" "$node_type" "$validator_address" "$host" "DISCOVERY_PORT_EXTERNAL" "$discovery_port")"
  ...
  if [[ "$role_group" == "consensus" && "$node_type" == "validator" ]]; then
    ...
    resolved_public_host="$(testnet_first_nonempty "$validator_mesh_host" "$local_ip" "$management_host" "$host" "$resolved_public_host")"
    public_address="$(compute_public_address "$resolved_public_host" "$public_p2p_port")"
    discovery_public_address="$(compute_public_address "$resolved_public_host" "$public_discovery_port")"
