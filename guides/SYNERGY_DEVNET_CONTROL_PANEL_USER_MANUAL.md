...
Generated by `scripts/devnet15/generate-wireguard-mesh.sh` with `WIREGUARD_PORT_BASE=51820`.
...
1. Generate full mesh configs from operator machine.
2. Apply only `node-01.conf` to first node.
3. Bring up WireGuard interface on machine-01.
...
# 3) generate mesh configs
./scripts/devnet15/generate-wireguard-mesh.sh

# 4) verify machine-01 config exists
ls devnet/lean15/wireguard/configs/node-01.conf

# 5) if machine-01 is remote, copy config to that host
scp devnet/lean15/wireguard/configs/node-01.conf <user>@<machine-01-host>:/tmp/synergy-devnet.conf

# machine-01 host
sudo mkdir -p /etc/wireguard
sudo install -m 600 /tmp/synergy-devnet.conf /etc/wireguard/synergy-devnet.conf
sudo wg-quick down synergy-devnet || true
sudo wg-quick up synergy-devnet
sudo wg show
```

After the first node is up, continue bringing remaining machines online in deterministic order (Section 8.6).

If machine-01 is the same device as the operator machine, you can skip `scp` and install directly:
...
1. Install WG tools.
2. Apply that machine's generated `.conf`.
3. Bring interface up.
4. Verify `wg show`.
5. Start node process.
...
| --- | --- | --- |
| WG interface name | `synergy-devnet` | Matches orchestration defaults |
| Node 1 config file | `node-01.conf` | Deterministic first validator |
| Remote config path | `/etc/wireguard/synergy-devnet.conf` | Matches automation scripts |
...
### 8.3 Generate WireGuard mesh artifacts
...
- `devnet/lean15/wireguard/keys/<machine-id>/`
...
### 8.5 Bring up WireGuard across fleet

Recommended bulk sequence:

1. `wireguard_install` on each machine that already has a valid `MACHINE_XX_HOST`
2. `wireguard_connect` on those same machines
3. `wireguard_status` on those same machines
...
| `bootstrap_node` | Install + WG + start | `admin` |
| `wireguard_install` | Install WG tools | `admin` |
| `wireguard_connect` | Apply config and bring tunnel up | `admin` |
| `wireguard_disconnect` | Bring tunnel down | `admin` |
| `wireguard_status` | Show tunnel state | `operator` |
| `wireguard_restart` | Down/up tunnel | `admin` |
...
| --- | --- | --- |
| Tunnel bootstrap 1 | `wireguard_install` | `all` |
| Tunnel bootstrap 2 | `wireguard_connect` | `all` |
| Tunnel verification | `wireguard_status` | `all` |
...
First operator is responsible for generating and distributing mesh configs.

### 13.2 Default orchestrator behavior

- WG interface default: `synergy-devnet`
- Remote conf path: `/etc/wireguard/synergy-devnet.conf`
- Activation: `wg-quick up synergy-devnet`

Per-machine overrides in `hosts.env`:

- `MACHINE_XX_WG_INTERFACE`
- `MACHINE_XX_WG_REMOTE_CONF`
...
- Missing config: regenerate mesh.
- Missing `wg-quick`: install WG tools.
...
- Admin required for install/bootstrap/WireGuard connect-disconnect-restart.
