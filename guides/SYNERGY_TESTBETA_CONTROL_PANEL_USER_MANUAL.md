...
Version: 2026-03-06
Applies to: `tools/testbeta-control-panel` (desktop app + `testbeta/lean15` closed-testbeta profile, 23 active node slots across 13 physical machines)

## Table of Contents

1. Scope and Non-Negotiable Testnet-Beta Rules
2. Architecture and Workspace Layout
3. Closed Testnet-Beta Configuration Reference
...
8. First Operator Runbook (Bootstrap the Private Testnet-Beta)
9. Subsequent Operator Runbook (Join and Operate Existing Testnet-Beta)
...
16. Testnet-Beta Test Phases and Validation Commands
...
This manual is the operational reference for the Synergy **closed testbeta**. It is not a public testnet runbook.

Closed-testbeta requirements enforced in this profile:
...
- **Determinism**
  ...
  - Deterministic genesis generation (`scripts/testbeta/generate-testbeta-genesis.sh`)
  - Determinism check (`scripts/testbeta/check-determinism.sh`)
...
- `testbeta/lean15/wireguard/`
- `scripts/testbeta/`
- `scripts/reset-testbeta.sh`
- `guides/SYNERGY_TESTBETA_CONTROL_PANEL_USER_MANUAL.md`

If `testbeta/lean15/hosts.env` does not exist, it is copied from `hosts.env.example`.
...
- Rendered configs: `testbeta/lean15/configs/node-*.toml`
- Genesis generator: `scripts/testbeta/generate-testbeta-genesis.sh`
- Active genesis: `config/genesis.json`

### 3.1 Global network values

- Chain ID: `338639`
- Network ID/name (config): `338639` / `synergy-testbeta-closed`
- Genesis network ID (metadata): `synergy-testbeta-closed-001`
...
### 4.2 Public-facing testbeta service endpoints (Atlas/clients)
...
- `<workspace>/testbeta/lean15/hosts.env`
- `<workspace>/testbeta/lean15/node-inventory.csv`
- `<workspace>/testbeta/lean15/wireguard/configs/`
- `<workspace>/testbeta/lean15/wireguard/keys/`
- `<workspace>/scripts/testbeta/remote-node-orchestrator.sh`

Do not move generated WireGuard files into ad-hoc directories like `networking/wireguard/...` unless you also update automation to match. The orchestrator and monitor expect the canonical `testbeta/lean15/wireguard/...` paths.
...
./scripts/testbeta/remote-node-orchestrator.sh node-01 setup_node
./scripts/testbeta/remote-node-orchestrator.sh node-01 status
```
WORKSPACE="${HOME}/.synergy-node-control-panel/monitor-workspace"
sudo install -m 600 "$WORKSPACE/testbeta/lean15/wireguard/configs/node-01.conf" /etc/wireguard/synergy-testbeta.conf
...
1. `sudo wg show synergy-testbeta` -> `No such file or directory`
...
- App bundle: `tools/testbeta-control-panel/control-service/target/release/bundle/macos/Synergy Node Control Panel.app`
...
```bash
cd tools/testbeta-control-panel
npm ci
...
```powershell
cd tools\testbeta-control-panel
npm ci
npm run dist:electron
```
...
2. Confirm inventory path resolves to `<workspace>/testbeta/lean15/node-inventory.csv`.
...
5. Confirm `<workspace>/testbeta/lean15/hosts.env` exists.
...
- Installer bundles present: `testbeta/lean15/installers/node-01`..`node-25`
...
| `ATLAS_BASE_URL` | Recommended | `https://testbeta-explorer.synergy-network.io` | Atlas link base |
...
mkdir -p ~/.ssh
chmod 700 ~/.ssh
ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519 -C "synergy-testbeta-ops" -N ""
...
ssh-keygen -t ed25519 -f "$env:USERPROFILE\.ssh\id_ed25519" -C "synergy-testbeta-ops" -N '""'
```
...
```bash
sudo wg show
ip addr | rg -n "synergy-testbeta|wg"
```
...
Set in `testbeta/lean15/hosts.env`:
...
- `testbeta/lean15/observability/docker-compose.yml`
...
3. re-render configs
4. validate closed-testbeta constraints
...
### 18.6 `sudo wg show synergy-testbeta` says `No such file or directory` (macOS)
...
- Node snapshots: `testbeta/lean15/reports/node-monitor-exports/`
- Remote exports: `testbeta/lean15/reports/remote-exports/<machine-id>/`
