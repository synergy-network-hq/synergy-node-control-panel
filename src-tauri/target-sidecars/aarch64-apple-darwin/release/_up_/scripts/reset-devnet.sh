...
# Synergy Devnet Reset Script
# This script stops all node processes, resets the blockchain to block 0, and restarts the devnet

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
BINARY="$PROJECT_DIR/target/release/synergy-devnet"
DATA_DIR="$PROJECT_DIR/data"
PID_FILE="$DATA_DIR/synergy-devnet.pid"

if [[ -z "${SYNERGY_LEGACY_RESET:-}" && -x "$PROJECT_DIR/scripts/devnet15/reset-devnet.sh" ]]; then
    exec "$PROJECT_DIR/scripts/devnet15/reset-devnet.sh" "$@"
fi
...
kill_synergy_processes() {
    ...
    pids=$(pgrep -x "synergy-devnet" 2>/dev/null || true)
    for pid in $pids; do
        stop_pid "$pid"
    done
}
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Synergy Devnet Reset Script${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
# Step 1: Kill all synergy-devnet processes
echo -e "${YELLOW}[1/3] Stopping all synergy-devnet processes...${NC}"
...
# Kill any remaining synergy-devnet processes (avoid broad pkill that can kill this script)
kill_synergy_processes
...
# Find all synergy-devnet processes that might be bootnode1
# Use timeout to prevent hanging if pgrep has issues
PGREP_OUTPUT=$(timeout 3 pgrep -f "synergy-devnet.*bootnode\|synergy-devnet.*--config.*bootnode1" 2>/dev/null || echo "")
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  Devnet reset complete!${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo -e "View logs: ${BLUE}tail -f $DATA_DIR/logs/bootnode1.out${NC}"
echo -e "Check status: ${BLUE}$PROJECT_DIR/devnet.sh status${NC}"
