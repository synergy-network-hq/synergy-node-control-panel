...
pub async fn testbeta_get_live_status() -> Result<TestnetBetaLiveStatus, String> {
    ...
    let (discovery_status, discovery_detail) =
        discovery_summary(healthy_bootnodes, bootnodes.len(), healthy_seed_servers, seed_servers.len());
    ...
    if matches!(role.id.as_str(), "rpc_gateway" | "indexer") {
        ...
        let nginx_contents = format!(
            "# Nginx reverse proxy for {role_display} ({node_id})\n\
             ...
             # Deploy: sudo cp this-file.conf /etc/nginx/sites-available/{rpc_subdomain}.conf\n\
             #         sudo ln -sf /etc/nginx/sites-available/{rpc_subdomain}.conf /etc/nginx/sites-enabled/\n\
             #         sudo nginx -t && sudo systemctl reload nginx\n\
             # SSL: sudo certbot --nginx -d {rpc_subdomain} -d {ws_subdomain} --expand\n\n\
             ...
             # P2P port {p2p_port} must be open in your firewall for peer connectivity.\n\
             # Server public IP: {public_host}\n",
...
fn chain_summary(
    ...
    if healthy_bootnodes > 0 || healthy_seed_servers > 0 {
        return (
            ...
            "Bootstrap endpoints are reachable, but the public RPC is not answering yet.".to_string(),
        );
    }
...
fn running_pid_for_workspace(workspace_directory: &Path) -> Option<u32> {
    let pid_path = workspace_directory.join("data").join("synergy-testbeta.pid");
...
fn resolve_testbeta_runner(
    ...
    for root in app_context.resource_roots() {
        for manifest in [root.join("synergy-testnet-beta").join("src").join("Cargo.toml")] {
            if manifest.is_file() {
...
fn recent_launch_failure_detail(workspace_directory: &Path, subcommand: &str) -> Option<String> {
    let candidates = [
        ...
        workspace_directory.join("logs").join("synergy-testbeta.log"),
    ];
...
async fn query_public_chain_height(client: &Client) -> Result<u64, String> {
    let value = query_rpc_value(client, TESTNET_BETA_PUBLIC_RPC_ENDPOINT, "synergy_blockNumber", json!([])).await?;
    parse_rpc_u64(value)
}
...
mod tests {
    ...
    fn setup_node_writes_role_metadata_and_bootstrap_inputs() {
        with_temp_home(|_| {
            ...
            assert_eq!(bootnodes[0].as_str(), Some("bootnode1.synergynode.xyz:38638"));
            assert_eq!(bootnodes[1].as_str(), Some("bootnode2.synergynode.xyz:38638"));
            assert_eq!(bootnodes[2].as_str(), Some("bootnode3.synergynode.xyz:38638"));
            ...
            assert_eq!(seeds[0].as_str(), Some("http://seed1.synergynode.xyz:18080"));
            assert_eq!(seeds[1].as_str(), Some("http://seed2.synergynode.xyz:18080"));
            assert_eq!(seeds[2].as_str(), Some("http://seed3.synergynode.xyz:18080"));
            ...
            assert_eq!(dns_records[0].as_str(), Some("_dnsaddr.bootstrap.synergynode.xyz"));
            ...
            assert_eq!(peer_bootnodes[0].as_str(), Some("bootnode1.synergynode.xyz:38638"));
    ...
    fn resolve_runner_falls_back_to_generic_platform_binary() {
        with_temp_home(|home| {
            ...
            match runner {
                ...
                other => panic!("expected binary runner fallback, got {:?}", std::mem::discriminant(&other)),
            }
        });
    }
